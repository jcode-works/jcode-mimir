...
function nextActions(input) {
    ...
    if (input.chunksIndexed === 0 || input.missingFromIndex > 0 || input.staleInIndex > 0) {
        ...
        steps.push("If files remain missing because they are scanned or image-only PDFs, configure `pdfOcrCommand` or convert scans/images to OCR text before ingesting.");
    }
    if (input.warnings > 0) {
