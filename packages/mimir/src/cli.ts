...
function printSetup(result: Awaited<ReturnType<typeof setupProject>>, title: string): void {
  ...
  if (result.ingested) {
    console.log(
      `  - ingested indexedFiles=${result.ingested.indexedFiles} chunks=${result.ingested.chunks} skippedFiles=${result.ingested.skippedFiles} errors=${result.ingested.errors.length}`,
    )
